...
public class UserRegistration extends Activity {
	...
	String usernameStr, passwordStr, myNameStr, myPhoneStr;
	String reply;

	private static final String SOAP_ACTION = "http://10.0.2.2:8080/soap/UserRegistrationService";
	...
	private void linkComponents() {
		...
		submitBtn = (Button) findViewById(R.id.submitForm);
	...
	private void loadValues() {
		...
		myNameStr = myName.getText().toString();
		myPhoneStr = myPhone.getText().toString();
	...
	class XMLAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			SoapObject request = new SoapObject(WSDL_TARGET_NAMESPACE,
					OPERATION_NAME);
			request.addProperty("recduser_name", myNameStr);
			request.addProperty("recdlogin_id", usernameStr);
			request.addProperty("recdpassword", passwordStr);
			request.addProperty("recdphn_number", myPhoneStr);
